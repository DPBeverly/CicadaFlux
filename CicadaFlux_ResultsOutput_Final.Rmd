...
##// This Initial part of the script is used for data management purposes have been LiCor 8100 files are combined, and corrected for collar spacing, as well as compiled with soil moisture and associate meta data. The complete and compiled files used throughout the analysis are available via Zenodo and codes are available via GitHub. The raw 8100 files are available via emailing the dbeverl@iu.edu
...
library(sf)
#library(raster)
library(geoR) # If you have a mac, you may have issues with this package. It requires XQuartz to be installed, which is not natively included in some versions of MacOS. 
library(gstat)
library(fields)
#library(rgdal) #rgdal is no longer available on CRAN. If you do not have installed, you can install a legacy version from the CRAN archive, which are available here: https://cran.r-project.org/src/contrib/Archive/rgdal/

#Note: the rgdal package will require GDAL to be installed on your machine. You can usually install it through your system's package manager (like apt-get on Ubuntu, brew on macOS, or conda for Conda environments).
library(terra)
library(tidyverse)
library(maptools) 
library(tmap)
...
##//Collar shapefiles
MMSF_pheno <- vect(paste0(dirname(rstudioapi::getActiveDocumentContext()$path), "/PhenoHoles_MMSF_v2.shp"))

###//Plot boundary
MMSF_pheno_bound <- vect(paste0(dirname(rstudioapi::getActiveDocumentContext()$path), "/PhenoHoles_Dissolve_20_v2.shp"))
